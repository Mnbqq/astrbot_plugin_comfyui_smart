...
| `/反推`（别名 `/识图` `/img2prompt`） | 看图反推提示词（发图或回复图片） | 按配置 |
| `/统计` | 查看出图统计 | 所有人 |
| `/刷新模型` | 重新读取模型清单 | **管理员** |
...
| `--negative` | `负面` | 覆盖负面提示词 |
| `--hires` | `放大` | Hires Fix 放大倍数，如 `--hires 1.5`；`--hires 0` 表示本次关闭 |
...
| `hires` | Hires Fix 开关、放大倍数、第二轮重绘幅度与步数、放大算法 |
...
```
ComfyUI 智能绘图 v0.6.0 已激活｜模板 3 个｜数据目录 …｜插件专属日志 可用｜Pages 已注册｜排队补偿上限 10 个任务
```
...
**已完成**：Hires Fix / 放大（v0.4.0）、看图反推提示词（v0.5.0）、图生图（v0.6.0）。
